...
function Zone(body, options, callback) {
  ...
  self.__finalize__ = function() {
    ...
    if (!self.__child_list_tail__) {
      ...
      if (!isRoot) {
        if (callback)
          scheduler.enqueueCallback(self.parent, self.parent, callback, [error].concat(result || []));
        ...
        for (var errorZone = error.zone;
             errorZone.parent;
             errorZone = errorZone.parent) {
          console.error('\nIn zone:', errorZone.stack);
        }
      ...
      assert(refCount === 0 || error || result);
  ...
  self.__register__ = function(child) {
    ...
    if (!scheduled && (refCount === 0 || error || result))
      enqueueFinalize();
  };
