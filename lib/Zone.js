...
function Zone(body, options, callback) {
  ...
  options.name = options.name || (body && body.name) || 'Anonymous zone';
  ...
  function enter() {
     assert(!closed);
     assert(enterCount === 0);
     // TODO(bert): I forgot what this assert was for but it breaks things.
     //             Figure out what it was for or drop it definitively.
     // assert(scheduled);
