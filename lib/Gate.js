...
function Gate(fn, source) {
  ...
  if (target === source) {
    ...
    target._ref(!source ? true : source, false);
  ...
  self.schedule = function(function_, this_, arguments_) {
    if (!zone)
      zone = root;
    if (!zone.childOf(source)) {
      // TODO improve error message
      throw new Error('This gate cannot be used from this zone');
    }

    debug('gate %s schedule %s in %s',
      self.name, function_._zoneName || function_.name, target.name);
  ...
  self.close = function() {
    ...
    target._ref(false, !source ? true : source);
  };
