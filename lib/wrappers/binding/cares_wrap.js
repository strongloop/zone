module.exports = function(binding) {
  ...
  if (isv010) {
    wrapCaresQuery = function(baseMethod) {
      return function query(name, callback) {
        ...
        callback = zone.bindAsyncCallback(callback,
                                          null,
    ...
    wrapCaresQuery = function(baseMethod) {
      return function query(req) {
        // c-ares sometimes make synchronous callbacks, so we have to do our
        // zone wrapping before invoking the binding layer.
        req.oncomplete = zone.bindAsyncCallback(req.oncomplete,
                                                null,
                                                { name: baseMethod.name });
  ...
  function wrapOnCompleteCallback(req, name) {
    ...
    if (req.oncomplete) {
      req.__wrapped_oncomplete__ = zone.bindCallback(req.oncomplete,
                                                     null,
                                                     { name: name });
    }
  ...
  function captureUserOnCompleteCallback(callback) {
    ...
    this.__wrapped_oncomplete__ = zone.bindCallback(callback,
                                                    null,
                                                    { name: this.__name__ });
  }
  ...
  return {
    ...
    getnameinfo: wrapLookup(cares_wrap.getnameinfo),
    ...
    getHostByAddr: wrapCaresQuery(cares_wrap.getHostByAddr),
