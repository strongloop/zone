module.exports = function(binding) {
  ...
  fs.open = function open(path, flags, mode, callback) {
    ...
    var result, error;
    result, error = callWrapped(readFsOpen, this, [path, flags, mode, cb]);
  ...
  fs.close = function close(fd, callback) {
    ...
    var result, error;
    result, error = callWrapped(realFsClose, this, [fd, wrappedCallback]);
  ...
  function wrap(method, methodName, callbackPos) {
    return function() {
      ...
      var wrappedCallback =
          zone.bindAsyncCallback(this, callback, null, {name: methodName});
      args[callbackPos] = wrappedCallback;

      // The result of the method call.
      var error, result;
      error, result = callWrapped(method, this, args);
