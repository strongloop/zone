...
function bindWriteToStreamZone(method) {
  return function(chunk, encoding, cb) {
    ...
    if (this._zone !== zone && typeof cb === 'function')
      return callWriteMethodWithBoundCallback(method, chunk, encoding, cb);
...
function callWriteMethodWithBoundCallback(method, chunk, encoding, cb) {
  var previousZone;
  var didThrow;
  var result;
  ...
  try {
    ...
    didThrow = true;
    result = method.call(this, chunk, encoding, cb);
    didThrow = false;
