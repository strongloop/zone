...
var fs = require('fs');
var AsyncListener = require('async-tracer').AsyncListener;

function ZoneListener(zone){
  this.zone = zone;
}
util.inherits(ZoneListener, AsyncListener);

AsyncListener.prototype.trackObject = function trackObject(obj){
  this.zone._registerChild(obj);
}

AsyncListener.prototype.releaseObject = function releaseObject(obj){
  this.zone._unregisterChild(obj);
}

AsyncListener.prototype.callbackCreated = function callbackCreated(name, id){
//  fs.writeSync(2, 'created ' + this.zone.name + ': ' + name + '\n');
  this.zone._incrementScheduledTaskCount();
}

AsyncListener.prototype.callbackReleased = function callbackReleased(name, id){
  // fs.writeSync(2, 'released ' + this.zone.name + ': ' + name + '\n');
  this.zone._decrementScheduledTaskCount();
}

AsyncListener.prototype.invokeCallback = function invokeCallback(next,name, id){
  var curZone = global.zone;
  global.zone = this.zone;
  try{
    next();
  } catch (err) {
    this.zone.throw(err);
...
function Zone(functionBody, options) {
   ...
   * ```asyncTracer.scheduleMicrotask``` or potential callbacks that will be
   * made from by I/O operations
   *
  ...
  this._asyncListener = new ZoneListener(this);
  ...
  if (!this._isRoot) {
    ...
    this._run(functionBody, options.arguments);
...
Zone.prototype.define = function define(functionBody, options, callback) {
  var options = _parseZoneOptions(options, callback);
...
Zone.prototype.apply = function apply(thisArg, fn, args) {
  ...
  if (global.zone === this) {
    ...
    var previousListener = asyncTracer.listener;
    
    global.zone = this;
    asyncTracer.listener = this._asyncListener;
    
    var result = this._apply(thisArg, fn, args);

    asyncTracer.listener = previousListener;
...
Zone.prototype.applyAsync = function applyAsync(thisArg, fn, args) {
  assert(!this._closed);
  asyncTracer.scheduleMicrotask(this._asyncListener, thisArg, fn, args);
};
 ...
 * @private
 */
Zone.prototype._run = function run(fn, args) {
  if (this._beforeTask) {
...
Zone.prototype.scheduleMicrotask = function(cb) {
  asyncTracer.scheduleMicrotask(this._asyncListener, this, cb);
  return null;
};
 ...
 * the queue of Zones to be cleaned up on the next iteration.
 *
...
Zone.prototype.enqueueFinalize = function enqueueFinalize() {
  ...
  if (!this._finalizerScheduled) {
    ...
    asyncTracer.scheduleMicrotask(this._asyncListener, this, this._finalize);
  }
};
...
Zone.prototype._finalize = function _finalize() {
  ...
  if (!this._exiting &&
      (this._numScheduledTasks > 1 || this._numNonLocalZoneCallbacks > 0)) {
    return;
  }
...
Zone.prototype._invokeCallbacks = function() {
  //fs.writeSync(2, 'zone exit: ' + this.name + '\n');  
  if (!this._isRoot) {
    if (this._errorFirstCallback) {
      // call the main callback
      asyncTracer.scheduleMicrotask(this.parent._asyncListener, this.parent,
        this._errorFirstCallback, [this._error].concat(this._result || []));
    } else if (!this._error && this._successCallback) {
      // success so call the success callback
      asyncTracer.scheduleMicrotask(this.parent._asyncListener, this.parent,      
        this._successCallback, this._result || []);
...
Zone.prototype.dump = function dump(options) {
  ...
  var ownDesc = util.format(
      ...
      (isAlive ? '+' : ' '), this.__id, this.name || 'anonymous',
  ...
  var child = this._lastZoneCallback;
  while (child) {
    childDesc = child.dump({indent: indent + 1}) + childDesc;
    child = child._previousSibling;
  }
